...
local function launcher(env, args)
	...
	env = env or ""
	args = args or ""
	awful.spawn.with_shell(table.concat({ term, env, LAUNCHER, args }, " "))
end
...
local globalkeys = gears.table.join(
  -- launchers
	...
	awful.key({ MODKEY }, "q", function()
		launcher("env PROVIDERS_FILE=power-menu.conf")
	end, { description = "power-menu", group = "launcher" }),

	awful.key({ MODKEY }, "c", function()
		launcher("env PROVIDERS_FILE=edit-config.conf")
	end, { description = "power-menu", group = "launcher" }),

	-- screenshots
	awful.key({}, "Print", function()
		awful.util.spawn("scrot -e 'mv $f ~/screenshots/ 2>/dev/null'", false)
	end, { description = "capture the whole screen", group = "screenshot" }),
